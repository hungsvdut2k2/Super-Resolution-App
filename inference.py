...
def main(args):
    ...
    sr_model = build_model(args=args)
...
def build_model(args) -> nn.Module:
    ...
    sr_model = model.__dict__[args.model_arch_name]()
    print(f"Build `{args.model_arch_name}` model successfully.")

    # Load model weights
    sr_model = load_pretrained_state_dict(
        sr_model, args.compile_state, args.model_weights_path
    )
    print(
        f"Load `{args.model_arch_name}` model weights `{os.path.abspath(args.model_weights_path)}` successfully."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inputs", type=str, default="./figure/comic.png", help="Original image path."
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        "--model_weights_path",
        type=str,
        default="/Users/viethungnguyen/SRGAN-PyTorch/results/pretrained_models/SRResNet_x4-SRGAN_ImageNet.pth.tar",
